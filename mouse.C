...
{
	int retcode;
	int xmin,xmax,ymin,ymax,x_max=625,y_max=479;
	int size;
...
void newmouse(int *nx,int *ny,int *nbuttons)
{
	...
	*nbuttons = buttonsn;
	if(buttons0 == *nbuttons)
		*nbuttons = 0;    //使得能连续按键
	if((xn == x0 && yn == y0 && buttonsn == buttons0)||MouseS>60)
		return;            //鼠标状态不变则直接返回S
...
/*	int size;
	...
	*/
//	IMbuffer=(char *)malloc(size);
	if(IMbuffer!=NULL)
		getimage(nx-1,ny-2,nx+11,ny+17,(void *)IMbuffer);
	else
	{
		printf("Error");
	/*	getchar();
...
void clrmous(int nx,int ny)//清除鼠标
{
	if(IMflag==1)
	{
		...
		mouse(nx,ny);
		putimage(nx-1,ny-2,IMbuffer,COPY_PUT);
	//	putimage(0,0,IMbuffer,COPY_PUT);
	//	free(IMbuffer);
		memset(IMbuffer,0,sizeof(IMbuffer));
		IMflag=0;
