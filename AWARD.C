...
RETURN:3,
***********************************************/
int page_award(int *x,int* pinfo_page, int psave[], int ifin[][6], int pos[], int* template_page,int *ifadd,int *count,int turn[])
{
	unsigned s;
	void* p;
	int i[5] = { 0 };
	...
	i[2] = i[1] + pos[6] * i[3];
	i[3] = pos[0] * i1 + pos[1] * i2 + pos[6] * i3 + pos[3] * i4;
	
	*template_page = 4;
	
	page_award_draw(*x, psave, ifin, pos);
	...
	while (1)
	{
		...
		else award_border(0, 0, 500, 90 + *x, 0);
		if (MouseX > 0 && MouseX < 500 && MouseY>90 + *x && MouseY < 155 + *x && pos[0] == 0)//求职意向 line(0,125+x+35-i[0],610,125+x+35-i[0]);
		{
			if (mouse_press(0, 90 + *x, 500, 160 + *x) == 2)
			{
				MouseS = 1;
				newmouse(&MouseX, &MouseY, &press);
				award_border(0, 97 + *x, 500, 157 + *x, 1);
			}
			if (mouse_press(0, 90 + *x, 500, 160 + *x) == 1)
			{
				*pinfo_page = 2;
				delay(100);
				return 11;
		...
		else award_border(0, 97 + *x, 500, 157 + *x, 0);
		if (MouseX > 0 && MouseX < 500 && MouseY>155 + *x - i[0] && MouseY < 311 + *x- i[0] && pos[1] == 0)//教育经历 line(0,266+x+45-i[1],610,266+x+45-i[1]);
		{
			if (mouse_press(0, 155 + *x - i[0], 500, 311 + *x - i[0]) == 2)
			{
				MouseS = 1;
				newmouse(&MouseX, &MouseY, &press);
				award_border(0, 163 + *x - i[0], 500, 306 + *x - i[0], 1);
			}
		...
		else award_border(0, 163 + *x - i[0], 500, 306 + *x - i[0], 0);
		if (MouseX > 0 && MouseX < 500 && MouseY>311 + *x - i[1]+add[0] && MouseY < 467 + *x - i[1]+add[0] && pos[6] == 0)//实习经历 line(0,382+x+55,610,382+x+55);
		{
			if (mouse_press(0, 311 + *x - i[1]+add[0], 500, 467 + *x - i[1]+add[0]) == 2)
			{
				MouseS = 1;
				newmouse(&MouseX, &MouseY, &press);
				award_border(0, 313 + *x - i[1]+add[0], 500, 462 + *x - i[1]+add[0], 1);
			}
			if (mouse_press(0, 311 + *x - i[1]+add[0], 500, 467 + *x - i[1]+add[0]) == 1)
			{
				*pinfo_page = 8;
				delay(100);
				return 11;
		...
		else award_border(0, 313 + *x - i[1]+add[0], 500, 462 + *x - i[1]+add[0], 0);
		if (MouseX > 0 && MouseX < 500 && MouseY>467 + *x - i[2]+add[0]+add[1] && MouseY < 575 + *x - i[2]+add[0]+add[1] && pos[3] == 0)//技能信息 line(0, 480 + x + 65 , 610, 480 + x+ 65 );
		{
			if (mouse_press(0, 467 + *x - i[2]+add[0]+add[1], 500, 575 + *x - i[2]+add[0]+add[1]) == 2)
			{
				MouseS = 1;
				newmouse(&MouseX, &MouseY, &press);
				award_border(0, 469 + *x - i[2]+add[0]+add[1], 500, 570 + *x - i[2]+add[0]+add[1], 1);
			}
		...
		else award_border(0, 469 + *x - i[2]+add[0]+add[1], 500, 570 + *x - i[2]+add[0]+add[1], 0);
		if (MouseX > 0 && MouseX < 500 && MouseY>575 + *x - i[3]+add[0]+add[1] && MouseY < 775 + *x - i[3]+add[0]+add[1] && pos[4] == 0)//自我评价 
		{
			if (mouse_press(0, 575 + *x - i[3]+add[0]+add[1], 500, 775 + *x - i[3]+add[0]+add[1]) == 2)
			{
				MouseS = 1;
				newmouse(&MouseX, &MouseY, &press);
				award_border(0, 577 +*x - i[3]+add[0]+add[1], 500, 770 + *x - i[3]+add[0]+add[1], 1);
			}
		...
		else award_border(0, 577 + *x - i[3]+add[0]+add[1], 500, 770 + *x - i[3]+add[0]+add[1], 0);
		if (MouseX > 0 && MouseX < 500 && MouseY>311-i[1]+*x && MouseY < 461-i[1]+*x && pos[5] == 0)//在校经历311-i[1]+x
		{
			if (mouse_press(0, 311-i[1]+*x, 500, 461-i[1]+*x) == 2)
			{
				MouseS = 1;
				newmouse(&MouseX, &MouseY, &press);
				award_border(0, 313-i[1]+*x, 500, 456-i[1]+*x, 1);
			}
			if (mouse_press(0, 311-i[1]+*x, 500, 461-i[1]+*x) == 1)
			{
				*pinfo_page = 7;
				delay(100);
				return 11;
		...
		else award_border(0, 313-i[1]+*x, 500, 456-i[1]+*x, 0);
		if (MouseX > 0 && MouseX < 500 && MouseY>467+*x -i[2]+add[0] && MouseY < 593+*x -i[2]+add[0] && pos[2] == 0)//工作经历467+x -i[2]+add[0]
		{
			if (mouse_press(0, 467+*x -i[2]+add[0], 500, 593+*x -i[2]+add[0]) == 2)
			{
				MouseS = 1;
				newmouse(&MouseX, &MouseY, &press);
				award_border(0, 469+*x -i[2]+add[0], 500, 588+*x -i[2]+add[0], 1);
			}
		...
		else award_border(0, 469+*x -i[2]+add[0], 500, 588+*x -i[2]+add[0], 0);
		...
		//实习经历删除框rectangle(580,316+x-i[1],600,336+x-i[1]);
		if (MouseX > 580 && MouseX < 600 && MouseY > 346 + *x - i[1] && MouseY < 366 + *x - i[1] && pos[6] == 0)
		{
			if (mouse_press(580, 346 + *x - i[1], 600, 366 + *x- i[1]) == 2)
			{
				MouseS = 1;
				newmouse(&MouseX, &MouseY, &press);
			}
			if (mouse_press(580, 346 + *x - i[1], 600, 366 + *x - i[1]) == 1)
			{
				clrmous(MouseX, MouseY);
				pos[5] = 1;
				psave[6] = 0;
		...
		if (MouseX > 580 && MouseX < 600 && MouseY > 492 + *x - i[2] && MouseY < 512 + *x - i[2] && pos[3] == 0)
		{
		...
		if (MouseX > 580 && MouseX < 600 && MouseY > 580 + *x - i[3] && MouseY < 600 + *x - i[3] && pos[4] == 0)
		{
			if (mouse_press(580, 580 +*x - i[3], 600, 600 + *x - i[3]) == 2)
			{
				MouseS = 1;
				newmouse(&MouseX, &MouseY, &press);
			}
			if (mouse_press(580, 580 + *x - i[3], 600, 600 + *x - i[3]) == 1)
			{
				clrmous(MouseX, MouseY);
				pos[4] = 1;
				psave[5] = 0;
		...
		//工作信息删除框 rectangle(580, 517+x -i[2]+add[0], 600, 537+x -i[2]+add[0]);
		if (MouseX > 580 && MouseX < 600 && MouseY > 517+*x -i[2]+add[0] && MouseY < 537+*x -i[2]+add[0] && pos[2] == 0)
		{
			if (mouse_press(580, 517+*x -i[2]+add[0], 600, 537+*x -i[2]+add[0]) == 2)
			{
				MouseS = 1;
				newmouse(&MouseX, &MouseY, &press);
			}
			if (mouse_press(580, 517+*x -i[2]+add[0], 600, 537+*x -i[2]+add[0]) == 1)
			{
				clrmous(MouseX, MouseY);
				pos[2] = 1;
				psave[3] = 0;
...
RETURN:void
***********************************************/
void page_award_draw(int x, int psave[], int ifin[][6], int pos[])
{
	int i1 = 65, i2 = 151, i3 = 126, i4 = 108;
	int i[5] = { 0 };
	int add[2] = {0};
	...
	i[0] = pos[0] * i1;
	i[1] = pos[0] * i1 + pos[1] * i2;
	i[2] = pos[0] * i1 + pos[1] * i2 + pos[6] * i3;
	i[3] = pos[0] * i1 + pos[1] * i2 + pos[6] * i3 + pos[3] * i4;
	clrmous(MouseX, MouseY);
	//cleardevice();
	...
	{
		setlinestyle(0, 0, 3);
		line(0, 266 + x + 45 - i[1]+add[0], 610, 266 + x + 45 - i[1]+add[0]);
		puthz(0, 271 + x + 45 - i[1]+add[0], "实习经历", 24, 32, BLACK);
		setlinestyle(0, 0, 1);
		setlinestyle(0, 0, 1);
		rectangle(580, 336 + x - i[1]+add[0], 600, 356 + x - i[1]+add[0]);
		line(585, 341 + x - i[1]+add[0], 595, 351 + x - i[1]+add[0]);
		line(595, 341 + x - i[1]+add[0], 585, 351 + x - i[1]+add[0]);
	}
	...
	if (pos[2] == 0)
	{
		...
		setlinestyle(0, 0, 1);
		rectangle(580, 517+x -i[2]+add[0], 600, 537+x -i[2]+add[0]);
		line(585, 522+x -i[2]+add[0], 595, 532+x -i[2]+add[0]);
		line(595, 522+x -i[2]+add[0], 585, 532+x -i[2]+add[0]);
	}
	...
	{
		put_intern_info(346 + x - i[1]+add[0], ifin);
	}
	if (psave[4] == 1 && pos[3] == 0)
	{
		put_skill_info(495 + x - i[2]+add[0]+add[1], ifin);
	}
	if (psave[5] == 1 && pos[4] == 0)
	{
		put_comment_info(605 + x - i[3]+add[0]+add[1], ifin);
	}
...
RETURN:void
***********************************************/
void award_border(int x1, int y1, int x2, int y2, int x)
{
	setlinestyle(0, 0, 1);
	if (x == 1)
	{
